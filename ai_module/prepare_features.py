...
Her hastalık için ayrı pencere/ufuk:
  Apnea:   X=10dk (600s),  Y=5dk  (300s), adım=30s
  Cardiac: X=30dk (1800s), Y=15dk (900s), adım=60s
  Sepsis:  X=60dk (3600s), Y=60dk (3600s), adım=60s

Özellikler (her sinyal için):
  mean, std, min, max, son değer, trend slope
  + SpO2 %time<90, RR %time<30, HR HRV (std)
  + ECG beats: rr_mean_ms, rr_std_ms, rmssd_ms, pnn50,
               amp_mean, amp_std, amp_min, amp_slope

Label tanımı:
  Pencere sonu t'den sonraki Y saniye içinde label_X=1 var mı?
"""
...
CONFIGS = {
    "apnea":   {"x_sec": 1200, "y_sec": 300,  "step_sec": 30},
    "cardiac": {"x_sec": 1800, "y_sec": 900,  "step_sec": 60},
    "sepsis":  {"x_sec": 3600, "y_sec": 3600, "step_sec": 60},
}


# ─── ECG BEAT EKSTRAKSİYONU ──────────────────────────────────────────────────

def extract_ecg_beats(ecg_mv: np.ndarray, fs: int = ECG_HZ) -> pd.DataFrame:
    """
    R-peak tespiti (scipy.find_peaks) → per-beat DataFrame.
    Returns: timestamp_sec, rr_interval_ms, r_amplitude
    """
    # Adaptif eşik: üst çeyrek (P/T dalgaları R-peak'ten çok daha küçük)
    height_thr = np.percentile(ecg_mv, 80)
    min_dist   = max(3, int(fs * 60 / 300))   # 300bpm → 5 sample @ 25Hz
    ...
    rr_ms = np.diff(peaks) / fs * 1000.0   # saniye → ms

    rows = []
    for i, idx in enumerate(peaks[1:]):
        rows.append({
            "timestamp_sec":  round(idx / fs, 4),
            "rr_interval_ms": round(float(rr_ms[i]), 2),
            "r_amplitude":    round(float(ecg_mv[idx]), 4),
        })
...
def load_ecg_beats(pid: str, data_dir: str) -> pd.DataFrame | None:
    """
    Compact beats dosyasını yükler; yoksa ham ECG'den üretip kaydeder.
    """
...
def ecg_window_features(beats: pd.DataFrame, t_start: float, t_end: float) -> dict:
    """
    [t_start, t_end) aralığındaki beatlerden 8 özellik üretir.
    """
...
# ─── VITAL SIGN FEATURE EKSTRAKSİYONU ───────────────────────────────────────
...
def extract_features(hr: np.ndarray, spo2: np.ndarray, rr: np.ndarray) -> dict:
    ...
    # Ani SpO2 düşüş hızı: en negatif tek adım (apnea=derin ani, sepsis=yavaş kayma)
    ...
    # Nefes durması: RR < 10 oranı (apnea=yüksek, sepsis=düşük)
    feats["rr_pct_below_10"] = float(np.mean(rr_c < 10)) if len(rr_c) > 0 else np.nan
    # Periodic breathing: RR değişkenlik katsayısı (apnea=yüksek, sepsis=stabil)
    rr_mean = float(np.mean(rr_c)) if len(rr_c) > 0 else 1.0
    feats["rr_cv"] = float(np.std(rr_c) / max(rr_mean, 1.0)) if len(rr_c) > 0 else np.nan
    # HR/RR oranı: apnea sırasında RR ~0 → oran çok yükselir; sepsis'te her ikisi de yüksek → oran orta
    hr_mean = float(np.mean(hr_c)) if len(hr_c) > 0 else np.nan
    feats["hr_rr_ratio"] = float(hr_mean / max(rr_mean, 1.0)) if (len(hr_c) > 0 and len(rr_c) > 0) else np.nan
...
def build_windows(patient_df: pd.DataFrame, disease: str,
                  x_sec: int, y_sec: int, step_sec: int,
                  beats_df: pd.DataFrame | None = None) -> list[dict]:
    """Tek hasta için sliding window örnekleri üretir."""
    ...
    for _, row in patient_df.iterrows():
        s   = int(row["timestamp_sec"])
        sig = row["signalType"]
        val = row["value"]
        if sig == "HEART_RATE":
            hr_arr[s]  = val
            lbl_arr[s] = int(row[label_col])
        elif sig == "SPO2":
            spo2_arr[s] = val
        elif sig == "RESP_RATE":
            rr_arr[s] = val
    ...
    for t_end in range(x_sec, max_sec - y_sec + 1, step_sec):
        ...
        feats["patient_id"] = patient_df["patientId"].iloc[0]
        feats["ga_weeks"]   = patient_df["gestationalAgeWeeks"].iloc[0]
        feats["pna_days"]   = patient_df["postnatalAgeDays"].iloc[0]
        feats["pma_weeks"]  = patient_df["pma_weeks"].iloc[0]
...
def main():
    ...
    for disease, cfg in CONFIGS.items():
        print(f"\n--- {disease.upper()} (X={cfg['x_sec']}s, Y={cfg['y_sec']}s, adım={cfg['step_sec']}s) ---")
        all_samples = []

        for pid in patients:
            pdata = df[df["patientId"] == pid].copy()

            # ECG beats: tüm hastalıklar için yükle (sepsis/apnea'da da faydalı)
            beats = load_ecg_beats(pid, args.output_dir) if use_ecg else None

            samples = build_windows(pdata, disease,
        ...
        pos = out_df["label"].sum()
        neg = len(out_df) - pos
        print(f"  Toplam: {len(out_df):,} pencere | pozitif={pos} ({pos/len(out_df):.1%}), negatif={neg}")
